...
public class Economy implements Initializable {
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		for(int row=0; row<aplane.getAeconomySeatCount()/8; row++) {
			for(int col=0; col<8; col++) {
				ImageView imageView = new ImageView("/img/economy.png");
				...
				if(seatlist!=null) {
					for(String temp : seatlist) {
						if(("E-"+button.getText()).equals(temp) ) {
		...
		int row = aplane.getAeconomySeatCount()/8;
		int remain = aplane.getAeconomySeatCount()%8;
		if(remain !=0 ) {
